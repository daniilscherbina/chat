...
std::mutex wait_message_lock;
// список клиентов
std::vector<SOCKET> clients;
// очередь неотправленных сообщений
std::queue<Message> wait_message;
...
void client_geter_thread(SOCKET s_new)
{
    ...
    while (true) {
        ...
        if (!std::string(B.text).compare("!exit"))
            break;
...
void client_sendler_thread()
{
    while (true)
    {
        ...
        if (ar != 0)
        {
            ...
            std::cout << "send [" << B.name << "]: " << B.text << std::endl;
